                ...
                {message.recommendations && message.recommendations.length > 0 && (
                  <div className="space-y-3 w-full">
                    {message.recommendations.map((rec) => (
                      <div
                        key={rec.tmdbId}
                        className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl p-4 hover:border-white/20 transition-all hover:scale-[1.02] cursor-pointer"
                      >
        ...
        <div className="p-6 border-t border-white/10">
          <div className="flex gap-2">
            <input
              ...
              disabled={isThinking}
              className="flex-1 bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-purple-500/50 disabled:opacity-50 transition-all"
            />
            <Button
              onClick={handleSendMessage}
              disabled={!inputValue.trim() || isThinking}
              className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-6 disabled:opacity-50"
            >
              <Send className="w-4 h-4" />
            </Button>
          </div>
          <p className="text-white/30 text-xs mt-2 text-center">
            Ask for movie or TV show recommendations
          </p>
        </div>
      </div>
    </>
